...
| **Contactos** | Agenda de NPCs (crear/editar requiere el permiso `contacts.edit`, que por defecto solo tienen los administradores) con datos globales (nombre, raza —desplegable guiado—, profesiones del catálogo, estado, foto, visibilidad total/oculto marcable ya desde la propia creación, notas del director —solo admin—, notas de contacto —visibles para cualquiera con acceso al contacto—) y datos por personaje (nivel de relación con etiqueta, tipo de relación —incluida la pertenencia a la Untersuchung, ver más abajo—, salario, notas privadas) sobre el **personaje activo** de cada usuario; importación/exportación Excel. Módulo integrado a partir del proyecto independiente [ContactosWH](https://github.com/jmsanesteban/ContactosWH) (ahora archivado, ver nota histórica en su README) |
| **Sistema de permisos** | Control granular por función (`professions`/`skills`/`pathfinder`/`characters`/`contacts`/`equipment`/`food`/`admin`): plantillas de permisos reutilizables con pantalla de checkboxes agrupados por módulo — este es el mecanismo recomendado; las asignaciones directas por usuario existen para excepciones puntuales, no como sustituto de una plantilla — más los administradores, que tienen acceso total siempre. **El navbar refleja los permisos reales de cada usuario** — una sección desaparece del menú (y sus rutas devuelven 403/redirigen a login) si falta el permiso correspondiente; un usuario sin sesión iniciada no ve ninguna sección, solo bienvenida + Entrar/Registrarse. Los usuarios que se registran reciben automáticamente la plantilla **"Jugador"** (ver/editar sus personajes, ver contactos visibles, ver/editar sus propios vínculos). La página de error 403 indica el permiso concreto que falta y qué hacer (pedir a un administrador que lo asigne), en vez de un "acceso denegado" genérico. El nombre de usuario es **insensible a mayúsculas/minúsculas** al iniciar sesión, registrarse o crear un usuario desde el admin ("Juanma" y "juanma" son la misma cuenta) |
...
| `tests/test_contacts_vinculos.py` | Directorio de Vínculos (`/contactos/vinculos`, ya no admin-only): requiere login, aviso si no hay personaje activo, filtrado por defecto al propio personaje activo (sin columna Personaje/Usuario), `?todos=1` amplía a todos los personajes de todos los usuarios (igual que ve siempre un admin), notas solo muestran recuento, búsqueda por nombre de contacto o personaje, y el endpoint JSON de búsqueda del popup **"Añadir vínculo"** (`/contactos/vinculos/buscar`): filtra por nombre o raza, sinónimos de género (enano/enana, elfo/elfa, humano/humana, ogro/ogra) sobre la última palabra escrita, y oculta los contactos no visibles a quien no es admin |
...
| **Ingredientes** | Solo dentro del Backup completo (sin página propia) | Nombre, vigor, moral, coste por docena, descripción y la matriz de compatibilidad (Sí/No/Condimento) con cada método de cocina, por nombre de método. Los métodos de cocina en sí siguen siendo un catálogo cerrado sembrado solo al arrancar el contenedor, no hace falta respaldarlos |
| **Bebidas** | Solo dentro del Backup completo (sin página propia) | Catálogo completo (origen, disponibilidad, calidad, sabor, precio en taberna, % por mayor, notas). Sigue siendo un catálogo cerrado sin CRUD propio en el admin — se incluye aquí para que una restauración desde cero no dependa de que `food_seed_service` ya haya corrido |
| **Recetas** | Admin → Comida y bebida → Exportar/Importar | Recetas propuestas por jugadores (pendiente/aprobada/rechazada) y las del libro, con método/ingredientes/condimentos por nombre y **su foto** (bytes en base64) |
...
1. Introduce el nombre, raza y género del personaje, sube opcionalmente una **foto/retrato**, y marca si es **miembro de la Untersuchung** (dato propio del personaje, con su(s) grado(s) — independiente de qué contactos considere él mismo de la Untersuchung, eso se marca por separado en el tipo de relación de cada vínculo). La foto se puede añadir o cambiar más tarde desde **Editar** en cualquier momento; si no tiene, la ficha muestra un icono de marcador de posición y el listado simplemente no muestra miniatura para ese personaje.
...
Ve a **Contactos** en el menú principal. Un contacto (NPC) tiene datos **globales** (nombre, raza, foto, carrera profesional con sueldo objetivo, estado, lugares de descanso/trabajo/ocio, notas del director — solo admin) y datos **por personaje** (nivel de relación, tipo de relación —incluida la pertenencia a la Untersuchung, ver más abajo—, notas privadas) sobre el vínculo con el **personaje activo** del usuario — cada personaje ve y edita solo su propio vínculo, nunca el de otro, aunque sean del mismo usuario.
...
- El listado de Contactos es global (visible para cualquier usuario, ya no hay filtrado por personaje): miniatura con lightbox al hacer clic, Nombre (enlace directo a la ficha — o a **Editar** si eres administrador, ver más abajo), Raza, Estado, y un botón con el nº de personajes vinculados que despliega su nivel y tipo en un panel con una paleta de color propia (teal), para diferenciarlo claramente del resto de la tabla — el botón queda resaltado mientras el panel está abierto. La ficha añade al final una tabla **"Personajes con relación"**, visible a cualquiera que pueda ver el contacto.
- **Foto sin recortes y navegación rápida en el listado** (2026-07-24): la miniatura ya no se ajusta con `object-fit:cover` (que a menudo cortaba la cabeza en retratos de cuerpo entero — las fotos de contacto varían mucho de encuadre) sino con `contain`, mostrando la foto completa dentro del círculo aunque no lo llene del todo; sin hover-zoom (se cortaba contra el borde de la tabla en pantallas estrechas, `table-responsive`), solo clic para el lightbox a pantalla completa. Pensado sobre todo para móvil: el **nombre del contacto** es ahora también un enlace (mismo aspecto que antes, solo añade el subrayado al pasar el ratón/tocar) para no tener que desplazar la tabla hasta el botón "Ver" — va a la ficha de solo lectura para cualquier usuario, o directo a **Editar contacto** si eres administrador (su destino habitual al pinchar un contacto). El botón "Ver" se mantiene igual, siempre a la ficha.
...
- **Paginación configurable por usuario** (2026-07-19; subido de 25 a **150** por defecto el 2026-07-24, para no tener que cambiar de página tan a menudo en móvil): el tamaño de página se cambia desde **Mi perfil → Preferencias → Contactos por página** (10/25/50/100/150/200) y se aplica la próxima vez que se visite el listado.
...
- **Añadir vínculo desde la propia página** (2026-07-24): si tienes un personaje activo, un botón **"Añadir vínculo"** abre un buscador emergente (avatar y nombre de tu personaje, campo de búsqueda con resultados en vivo) para crear el vínculo sin ir hasta la ficha del contacto. La búsqueda filtra por nombre **o raza**, con equivalentes de género (enano/enana, elfo/elfa, humano/humana, ogro/ogra) — como `Contact.raza` solo guarda la forma masculina, escribir cualquiera de las dos formas encuentra el mismo contacto. Al elegir un contacto y un nivel de relación (de 5 a -5) y confirmar, el vínculo se crea (o se actualiza el nivel si tu personaje ya tenía uno con ese contacto) y el popup se cierra sin salir de Vínculos.
...
- **Visibilidad en la ficha del personaje** (2026-07-19): el bloque "Equipamiento" (Tienda/Carrito/Inventario/Historial de compras) de la ficha solo se muestra si el usuario tiene el permiso `equipment.view` — igual que el resto de la app, ningún enlace debe llevar a un usuario a una sección que no puede ver. Los administradores lo ven siempre (bypass de permisos).
...
users
  ...
  └─ characters                   (perfil completo: características WFRP2, trasfondo del
       ...
       └─ career_visibility_grants → contact_career_visibilities (ver más abajo)
...
contact_character_links            (la visión de UN personaje sobre un Contacto —
  ...
  └─ tipo_relacion                 (JSON, lista de 0+ de Baza/Unter/Súbdito/Señor/Otra - Súbdito y
...
└── app/
    ...
    │   ├── contacts/
    │   │   ├── index.html         # Listado global (miniatura, raza, estado, nº vínculos)

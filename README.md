...
- **Proponer una receta nueva**: cualquier usuario puede proponer una receta desde **Comida y bebida → Proponer receta**. Eliges el método de cocina y hasta 4 ingredientes y 2 condimentos (el formulario solo deja elegir combinaciones válidas para ese método, según la tabla de compatibilidad); Vigor, Moral, Coste, Precio de compra, Duración, Recalentar, Complejidad y **Calidad** se **calculan automáticamente** con las mismas fórmulas del libro — no hay que rellenar nada de eso a mano, y el formulario muestra el cálculo en vivo según vas eligiendo. La **Calidad** no se elige: sale directamente de la Complejidad (1-4 Mala, 5-8 Normal, 9-12 Buena, 13+ Excelente — cada ingrediente relleno suma +1 a la complejidad, cada condimento +2, más la complejidad base del método). La receta queda en estado **pendiente** y no aparece en el catálogo público hasta que un administrador la revisa, le añade una imagen y la aprueba (o la rechaza, con un motivo). Desde **Comida y bebida → Mis recetas** puedes ver el estado de todo lo que has propuesto (pendiente/aprobada/rechazada, con el motivo si aplica). Si no se te ocurre un nombre, el botón **"Sugerir nombre"** junto al campo Nombre genera hasta 3 propuestas a partir del método y los ingredientes/condimentos ya elegidos y rellena el campo con una al azar si lo dejas vacío — puedes pulsar cualquiera de las alternativas mostradas para usarla en su lugar, o volver a pulsar "Sugerir nombre" para otra tanda aleatoria. Para que no suene a lista de la compra, cada categoría genérica del catálogo (Carne superior, Pescado inferior…) se traduce a una palabra concreta elegida al azar (venado/jabalí/buey/cordero, bacalao/arenque/anguila…), cada condimento a una frase atmosférica en vez de nombrarlo tal cual (Especias → "con un toque picante"/"con aroma especiado", Miel → "con aroma a miel"…), y cuando la mezcla cruza ingredientes de mar y de tierra se ofrece el término gastronómico real **"mar y montaña"** — p.ej. "Fusión de mar y montaña: venado y gambas con un toque picante" o "Tartar de venado y gambas con un toque picante" en vez de "Asado de carne superior y crustáceos con especias".
